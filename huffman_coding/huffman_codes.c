...
/**
 * print_codes -	recursively prints Huffman codes from a tree
...
static void print_codes(
	const binary_tree_node_t *node, char *path, size_t depth)
{
	symbol_t *sym;
...
/**
 * free_tree -	frees a Huffman tree (nodes and symbols)
...
/**
 * huffman_codes -	builds the Huffman tree and print codes for each symbol
...
int huffman_codes(char *data, size_t *freq, size_t size)
{
	...
	char path[512];						/* buffer for paths */

	if (!data || !freq || size == 0)
		return (0);

	root = huffman_tree(data, freq, size);	/* build the Huffman tree */
	if (!root)
		return (0);

	print_codes(root, path, 0);				/* print codes for each symbol */
	free_tree(root);
	return (1);
}
